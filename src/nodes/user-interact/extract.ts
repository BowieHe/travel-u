...
export async function extractAndUpdateTravelPlan(
    ...
    const lastAiMessage = messages.filter(msg => msg.getType() === "ai").slice(-1)[0];
    const lastHumanMessage = messages.filter(msg => msg.getType() === "human").slice(-1)[0];
...
最近的问答分析：
${lastAiMessage ? `AI最后的问题: "${lastAiMessage.content}"` : '没有AI问题（用户主动发起）'}
