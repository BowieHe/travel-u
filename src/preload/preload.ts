...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	// BrowserView 控制 API
	toggleBrowserView: (isOpen: boolean) => ipcRenderer.invoke("toggle-browser-view", isOpen),
	onBrowserViewLoading: (callback: (isLoading: boolean) => void) =>
		ipcRenderer.on("browser-view-loading", (_, isLoading) => callback(isLoading)),
	onBrowserViewError: (callback: (error: { errorCode: number; errorDescription: string }) => void) =>
...
declare global {
	interface Window {
		electronAPI: {
			...
			onBrowserViewError: (callback: (error: { errorCode: number; errorDescription: string }) => void) => void;
		};
	}
}
