...
export function getTripPlanSchema() {
    return z.object({
        destination: z.string().optional().describe("用户明确表达要去的目的地城市。只有当用户明确说'去XX'、'到XX'时才填写。"),
        departure: z.string().optional().describe("用户明确表达的出发城市。只有当用户明确说'从XX出发'、'XX出发'时才填写。"),
        startDate: z
            .string()
            .optional()
            .describe(
        ...
        budget: z
            .number()
            .optional()
            .describe("用户明确提到的预算数字。只有当用户明确提供预算金额时才填写。"),
