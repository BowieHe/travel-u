 ...
 */
export function createSafeSpecialistAgent(tools: DynamicStructuredTool[], systemMessage: string) {
    const baseAgent = createSpecialistAgent(tools, systemMessage);

    return async (state: AgentState): Promise<Partial<AgentState>> => {
        try {
            console.log(`Running specialist with ${state.messages.length} messages`);
            ...
            const contextMessages = state.messages.filter(
                (msg) =>
                    ...
                    (msg instanceof AIMessage && msg.content.toString().includes('task_prompt'))
            );

            console.log(`Filtered to ${contextMessages.length} context messages for specialist`);
