		...
		<>
			...
			>
				...
				{isOpen && (isLoading || error) && (
					<div className="absolute top-4 left-4 backdrop-blur-sm rounded-lg px-3 py-2 shadow-lg z-50 bg-travel-light border border-travel-accent">
						{isLoading && (
							<div className="flex items-center space-x-2 text-travel-primary">
				...
				{isOpen && (
					<button
						onClick={onToggle}
						className="absolute top-1/2 -right-10 transform -translate-y-1/2 border border-l-0 rounded-l-none rounded-r-lg p-3 transition-all duration-300 hover:scale-110 z-50 shadow-sm bg-travel-light border-travel-accent"
						title="收起搜索"
					>
						<ChevronsLeft
							size={20}
			...
			{!isOpen && (
				<button
					onClick={onToggle}
					className="fixed top-1/2 left-0 transform -translate-y-1/2 border border-r-0 rounded-r-lg rounded-l-none p-3 transition-all duration-300 hover:scale-110 z-40 shadow-sm bg-travel-light border-travel-accent"
					title="打开搜索"
				>
					<ChevronsRight size={20} className="text-travel-primary" />
				</button>
			)}
		</>
	);
