...
const electronAPI: ElectronAPI = {
	...
	sendMessage: (message: string) =>
		ipcRenderer.invoke(IPC_CHANNELS.AI_CHAT, message),
	onAIResponse: (callback: (response: string) => void) =>
		ipcRenderer.on(IPC_CHANNELS.AI_RESPONSE, (_, response) =>
			callback(response)
		),
	...
	onBrowserViewLoading: (callback: (isLoading: boolean) => void) =>
		ipcRenderer.on(IPC_CHANNELS.BROWSER_VIEW_LOADING, (_, isLoading) =>
			callback(isLoading)
		),
	onBrowserViewError: (
		callback: (error: {
